...
from Shinobu import Spotify, YouTube, app
from Shinobu.core.call import Shinobu
from Shinobu.utils import seconds_to_min, time_to_seconds
from Shinobu.utils.channelplay import get_channeplayCB
from Shinobu.utils.decorators.language import languageCB
from Shinobu.utils.decorators.play import PlayWrapper
...
from Shinobu.utils.inline.playlist import botplaylist_markup
from Shinobu.utils.logger import play_logs
from Shinobu.utils.stream.stream import stream
        ...
        if await YouTube.exists(url):
            ...
            if not config.SPOTIFY_CLIENT_ID or not config.SPOTIFY_CLIENT_SECRET:
                return await mystic.edit_text(
                    "Spotify isn't configured on this bot. Ask the owner to set it up."
                )
            if "track" in url:
                try:
...
async def anonymous_check(client, CallbackQuery):
    try:
        await CallbackQuery.answer(
            "You're an Anonymous Admin.\n\nGo to your group settings → Administrators → Click your name → Uncheck 'Remain Anonymous'.",
            show_alert=True,
        )
    except:
        return
...
@app.on_callback_query(filters.regex("ShinobuPlaylists") & ~BANNED_USERS)
@languageCB
...
async def slider_queries(client, CallbackQuery, _):
    ...
    med = InputMediaPhoto(
        media=f"https://img.youtube.com/vi/{vidid}/hqdefault.jpg",
