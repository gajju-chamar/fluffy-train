...
from Shinobu.utils.inline.playlist import (
    botplaylist_markup,
    get_playlist_markup,
    warning_markup,
)
...
@app.on_message(
    filters.command(PLAYLIST_COMMAND) & ~filters.edited & ~BANNED_USERS
)
@language
...
async def add_playlist(client, CallbackQuery, _):
    ...
    if len(_count) == SERVER_PLAYLIST_LIMIT:
        try:
            ...
            return
    title, duration_min, duration_sec, vidid = await YouTube.details(videoid, True)
